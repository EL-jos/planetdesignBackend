            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                        <h6 class="text-white text-capitalize ps-3">{{ $banner->exists ? "Modifier la categorie" : "Ajouter une categorie" }} </h6>
                    </div>
                </div>
                <div class="card-body pb-2">
                    <form method="post" action="{{ $banner->exists ? route('banner.update', $banner) : route('banner.store', $banner) }}" enctype="multipart/form-data">
                        @csrf
                            ...
                            @if($banner->image)
                                <div id="el-container-picture" style="width: 100%;">
                                    <div class="mb-3 el-container">
                                        <img src="{{ asset($banner->image->path) }}" alt="..." class="img-thumbnail">
                                        <button class="el-remove"
                                                hx-delete="{{ route('image.destroy', $banner->image) }}"
            ...
            function initializeFilepond(element) {
                if (!initializedElements.has(element)) {
                    FilePond.create(element, {
                        ...
                        server: {
                            process: {
                                url: '{{ route("uploadPhoto.banner", $banner) }}', // Remplacez par l'URL de votre action Laravel pour l'upload
                                method: 'POST',
